...
def UITodosLosPedidos(data):
    ...
    if len(df) > 0:
        ...
        # Opción 1: Usar índices
        
        # function with list of labels
        def search_orderid(searchterm: str) -> List[any]:
            if not searchterm:
                return []
            df.set_index(['ID', 'Seller'], inplace=True)
            
            pattern = f"(?i){re.escape(searchterm)}"
            mask = df.index.get_level_values('ID').str.match(pattern) | df.index.get_level_values('Seller').str.match(pattern)
