...
def safe_float(value):
    try:
        val = float(value)
        return val if not isnan(val) else 0
...
def safe_int(value):
    try:
        return int(float(value))  # Convierte cadenas o números a entero
    except (ValueError, TypeError):
        return 0
...
def UIdetalleBitacora(data_general, data_detalle, fees):
    ...
    if st.button('Generar PDF'):
        ...
        for i in range(len(data_detalle['suborder'])):
            ...
            # Obtener valores individuales de las columnas
            qty_of_packs_row = safe_int(data_detalle['qty_of_packs'][i])  # Convertir a int o asignar 0
            pack_price_row = round(safe_float(data_detalle['pack_price'][i]), 2) if qty_of_packs_row > 0 else 0
            discount_row = round(safe_float(data_detalle['discount'][i]), 2)
            subtotal_row = round(safe_float(data_detalle['subtotal'][i]), 2) if qty_of_packs_row > 0 else 0

            # Escribir datos en la tabla PDF
            pdf.cell(30, 5, f"{qty_of_packs_row}", align="C")
            pdf.cell(30, 5, f"${pack_price_row}", align="C")
            pdf.cell(20, 5, f"${discount_row}", align="C")
            pdf.cell(20, 5, f"${subtotal_row}", align="C")
            ...
            if data_detalle['estado'][i] != 'Cancelado':
                ...
                subtotal += round(safe_float(data_detalle.get('pack_price', [0])[i]), 2) * safe_int(data_detalle.get('qty_of_packs', [0])[i])
