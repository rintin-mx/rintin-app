...
def UIOrdenesAgruparDetalle(data,idPedido):
    ...
    for i, pedido in df.iterrows():
        ...
        with col5:
            ...
            objArry.append({
                ...
                "seller_name": pedido.product_name + '/' + pedido.sku,
    ...
    for i in range(len(objArry)):
        if objArry[i]['agrupadoSeleccion'] =='agrupado':
            ...
            agrupado_unicos.add(str(objArry[i]['order_id']))

        else:
            no_agrupado.append(objArry[i]['order_id'])
    
    agrupado_str = ', '.join(sorted(agrupado_unicos, key=int))


    #proceos para los ids Padres
    respuesta = False
    if len(agrupado) == len(objArry):
        trigger_btn = ui.button(text="Agrupar", key="trigger_btn")
    ...
    if respuesta:
        if len(agrupado)>0:
            ...
            with st.spinner(f'Actualizando estatus de los pedidos a Empaquetar...'):
                ...
                for order_id in agrupado_unicos:
                    r = asyncio.run(update_status_wordpress(order_id, order_status))
                    EventName,EventAction,EventUser='Agrupación','finalProcesoAgrupacion Se envio el pedido a "Empaquetar"',st.session_state.useremail
