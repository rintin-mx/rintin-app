                ...
                with col3:
                    if ordenes.estado == 'Empaquetar':
                        if st.button("Empaquetado", key=i):
                            ...
                            event_instert(EventName,EventAction,EventUser, ordenes.order_id)
...
def UIOrdenesEmpaquetarDetalle(data,idPedido):
    ...
    for i, pedido in df.iterrows():
        ...
        with col2:
            st.write("**Producto/SKU**")
            st.write(pedido.product_name + '/'+pedido.sku)   
        with col3:
            st.write("**Piezas**") 
            st.text(int(''.join(filter(str.isdigit, pedido.units_per_pack))))     
        with col4:
            st.write("**Qty(Paquetes)**") 
        ...
        with col5:
            ...
            objArry.append({"order_id":pedido.order_id,"seller_name":pedido.product_name + '/'+pedido.sku,
    ...
    if respuesta:
        if len(empaquetado)>0:
            ...
            with st.spinner(f'Actualizando estatus de los pedidos a Generar Guía...'):
                ...
                for order_id in empaquetado_unicos:
                    r = asyncio.run(update_status_wordpress(order_id, order_status))   
                    EventName,EventAction,EventUser='empaquetado','finalProcesoEmpaquetado Se envio el pedido a "Generar Guía"',st.session_state.useremail
