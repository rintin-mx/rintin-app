    ...
    if 'ordenCompraId' in st.session_state:
        if st.button('Generar PDF'):
            ...
            for value in st.session_state['dictProductos'][st.session_state['currentSeller']]:
                x = pdf.get_x()
                y = pdf.get_y()
                if len(str(value['nombre'])) > 30:
                    pdf.multi_cell(32, 5, str(value['nombre']), 1, align='C')
                else:
                    pdf.cell(32, 10, str(value['nombre']), 1, align='C')

                pdf.set_xy(x + 32, y)
                pdf.cell(12, 10, str(value['sku']), 1, align='C')
                ...
                pdf.cell(14, 10, str(value['tipo_producto']), 1, align='C')
