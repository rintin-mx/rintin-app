...
def update_order_metadata(order_id, num_guia, logis_op):
    ...
    try:
        ...
        if connection.is_connected():
            ...
            sql = "SELECT * FROM wp_postmeta WHERE post_id = %s AND meta_key = '_numero_guia'"
            cursor.execute(sql, (order_id,))
            print('primer select')
            print(cursor.rowcount)
            if cursor.rowcount == 0:
                sql = "INSERT INTO wp_postmeta (post_id, meta_key, meta_value) VALUES (%s, '_numero_guia', %s)"
                cursor.execute(sql, (order_id, num_guia))
            else:
                sql = "UPDATE wp_postmeta SET meta_value = %s WHERE meta_key = '_numero_guia' AND post_id = %s "
                cursor.execute(sql, (num_guia, order_id))

            sql = "SELECT * FROM wp_postmeta WHERE post_id = %s AND meta_key = '_logis_op'"
            cursor.execute(sql, (order_id,))
            print('segundo select')
            print(cursor.rowcount)
            if cursor.rowcount == 0:
                sql = "INSERT INTO wp_postmeta (post_id, meta_key, meta_value) VALUES (%s, '_logis_op', %s)"
                cursor.execute(sql, (order_id, logis_op))
            else:
                sql = "UPDATE wp_postmeta SET meta_value = %s WHERE meta_key = '_logis_op' AND post_id = %s "
...
select 
    ...
    group_concat(case when post_status = 'wc-generar_guia' then id else null end separator ', ') as hijos_guia
from 
	orders
...
where (postcode not in (select codigo_postal from cps) or order_id not in (select order_id from order_shipping))
        ...
        if len(wp_ordenes) > 0:
            wp_ordenes.columns = ['order_id', 'hijos_guia' , 'postcode', 'state', 'numero_guia', 'logis_op', 'ordenes_activas', 'num_hijos_guia']
